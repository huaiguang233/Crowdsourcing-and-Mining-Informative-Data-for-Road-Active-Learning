...
class CocoDataset(Dataset):
    ...
    def __init__(self, root_dir, set_name=None, transform=None, txt_file=None):
        """
        Args:
    ...
    def load_annotations(self, image_index):
        if hasattr(self, 'image_paths'):
            # Use the file name to match with COCO's annotations
            img_path = self.image_paths[image_index]
            img_name = os.path.basename(img_path)
            image_id = self.coco.getImgIds(imgIds=[img_name.split('.')[0]])[0]
        else:
