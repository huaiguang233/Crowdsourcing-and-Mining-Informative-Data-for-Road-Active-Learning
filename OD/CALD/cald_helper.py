...
def HorizontalFlip(image, bbox):
    ...
    # Convert tensor to PIL Image, then to OpenCV format (numpy array)
    image = F.to_pil_image(image)
    image = np.array(image)  # Convert to numpy array (OpenCV format)
...
def resize(img, boxes, ratio):
    ...
    # Convert tensor to PIL Image, then to OpenCV format (numpy array)
    img = np.array(img)  # Convert resized image to numpy array (OpenCV format)

    return img, boxes * ratio
...
def cutout(image, boxes, labels, cut_num=2, fill_val=0, bbox_remove_thres=0.4, bbox_min_thres=0.1):
    ...
    count = 0
    for _ in range(50):
        # Random cutout size: [0.05, 0.2] of original dimension
